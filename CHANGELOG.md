...
- **F108**: F107 facade completion — routed all four interfaces (CLI, TUI, HTTP, ACP) through `ports.WorkflowFacade.Run` + `RunSession.Events()` as the sole execution path, delivering F107's "single core" promise. The facade event model (`ports.Event` + `ProjectEvent`) is enriched with typed payload structs (`EnrichedStepPayload`, `EnrichedMessagePayload`, `EnrichedInputRequest`, `EnrichedTerminal`) carrying step name, message content, error message, duration, and InputRequest prompt losslessly from `transcript.ExchangeEvent` typed payloads (`StepPayload`, `MessagePayload`, `ToolPayload`) — no `transcript`/infra types leak into the domain layer. CLI `Adapter.Status` is now backed by `application.SessionRegistry.Get` plus a `history.db` fallback so `awf status <id>` resolves both live and completed runs (replacing the prior not-found stub); `awf list/history/resume/resume-list/validate` route through `cfg.Facade.*`. TUI run action calls `RunWorkflowViaFacade` and consumes `Events()` directly, eliminating the 200ms `tickMsg`/`executionPollMsg` poll loop. HTTP `POST /api/workflows/{scope}/{name}/run` executes via the facade and registers the returned `RunSession` in `application.SessionRegistry` (wired to `SessionLookup` via a thin adapter) so SSE subscribers resolve the session synchronously by run ID. ACP `dispatchViaFacade` is wired through `ACPSessionService.SetFacade` in `acp_serve.go` with one `WorkflowFacade` constructed per ACP session (per-session `stateStore` rooted at `acpSessionStateDir(sessionID)`) preserving the F105 state-isolation pattern; `projectFacadeEvents` projects `EventInputRequired` to a `session/update` carrying the prompt before parking, and `facadeEventToUpdate` now emits the actual assistant text instead of `ev.RunID`. Duplicate no-op recorders (`cli.nopRecorder`, `tui.tuiNopRecorder`) are consolidated into a single `transcript.NopRecorder` in `internal/infrastructure/transcript/`. Legacy execution paths deleted: ACP `runner.Run` branch in `HandleSessionPrompt`, `internal/infrastructure/acp/input_reader.go`, CLI `interactiveExec` + direct `execSvc.RunWithWorkflowAndRunID` call, TUI `bridge.RunWorkflow` + `tickMsg`/`executionPollMsg` + `scheduleTick`, and the legacy HTTP run path through `Bridge.StartExecution` (`Bridge.activeExecutions` is preserved for `List/Get/Cancel` metadata). `wiring_transcript.AttachMirrorSubscriber` migrated to consume `RunSession.Events()` keeping SC-001 ("only `Adapter.newSession` calls `Recorder.Subscribe`") clean. `.go-arch-lint.yml` updated for the new edges (`interfaces/cli → infrastructure/transcript`, `application → ports` for enriched payloads) and stale edges removed. Conformance goldens (`tests/fixtures/facade/{cli-stdout,acp-session-update,sse-frames,tui-tea-msg}.golden`) drive `TestFacadeConformance_4Interfaces`; `TestFacadeE2E_CtxCancelProducesWorkflowFailed` and `TestFacadeResume_RestoresState` are green; per-session/per-run ACP state isolation is verified by `TestACP_ConcurrentSessionsNoStateCollision` under `-race`. Iso-functional from the user's perspective — no output regression and no YAML schema change. Unblocks F109 (tools hardening).
...
- **F100**: Dedicated `roles/` namespace for agent roles — the environment variable for overriding role search paths is renamed from `AWF_AGENTS_PATH` to `AWF_ROLES_PATH`. All role discovery now happens exclusively under `roles/`-namespaced directories. Migration guide for users upgrading from F098:
  - **Environment variable**: rename `AWF_AGENTS_PATH` → `AWF_ROLES_PATH` in shell profiles, CI scripts, and Docker environments.
  - **Local project directory**: move `.awf/agents/<role>/` → `.awf/roles/<role>/`. The old `.awf/agents/` path is no longer searched.
  - **Cross-client local directory**: move `.agents/<role>/` (flat, at root) → `.agents/roles/<role>/`. Roles stored directly under `.agents/` without the `roles/` sub-namespace are no longer discovered.
  - **Cross-client global directory**: move `~/.agents/<role>/` → `~/.agents/roles/<role>/`. Same sub-namespace requirement as above.
  - **XDG global directory**: `$XDG_CONFIG_HOME/awf/roles/` path is unchanged (was not previously a dedicated path; roles were found via the old `AWF_AGENTS_PATH` override only).
  - Workflows referencing roles by name (e.g. `role: go-senior`) will fail at validation or execution time with `ErrRoleNotFound` until the role directories are migrated to the new namespace. Path-based refs (e.g. `role: ./path/to/role`) are unaffected as long as the referenced path still exists.
...
- **F090**: Plugin Event System — inter-plugin and core event reactivity via gRPC `EventService` with unary `HandleEvent` RPC; in-process `EventBus` with dot-segment glob pattern matching (`workflow.*`, `step.*`, `*.*`) and per-plugin buffered channels (256 events) for isolation; `ExecutionService` emits 7 lifecycle events (`workflow.started`, `workflow.completed`, `workflow.failed`, `step.started`, `step.completed`, `step.failed`, `step.retrying`); inter-plugin event routing via `HandleEventResponse.emitted_events` with cycle detection (max propagation depth 3); SDK `EventSubscriber` interface with `Patterns()` and `HandleEvent()` methods; `BasePlugin` no-op defaults preserve backward compatibility (existing plugins unaffected); plugin manifest `events` capability with `subscribe` and `emit` pattern declarations; `EventBus` wired through `InitSystem` → `RPCPluginManager.SetEventBus()` → `WithEventPublisher` SetupOption → `ExecutionService`
