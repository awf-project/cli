...
**Resolution:** Verify the role name matches a directory in one of the discovery paths (`.awf/roles/`, `.agents/roles/`, `$XDG_CONFIG_HOME/awf/roles/`, `~/.agents/roles/`) and that `<role-dir>/AGENTS.md` is readable. For path-based references, verify the path is correct relative to the workflow file. Set `AWF_ROLES_PATH=<dir>` to restrict discovery to a single directory.

**Example:**
```bash
awf run code-review
# Error [USER.INPUT.MISSING_ROLE]: role 'go-senior' not found in search paths:
#   .awf/roles/, .agents/roles/, ~/.config/awf/roles/, ~/.agents/roles/
```
