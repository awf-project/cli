...
func TestHTTPStatus_HappyPath(t *testing.T) {
	tests := []struct {
		name     string
		code     domainerrors.ErrorCode
		expected int
		...
		{
			name:     "USER.INPUT.MISSING_FILE returns 400",
			code:     domainerrors.ErrorCodeUserInputMissingFile,
			expected: 400,
		},
		{
			name:     "USER.INPUT.VALIDATION_FAILED returns 400",
			code:     domainerrors.ErrorCodeUserInputValidationFailed,
			expected: 400,
		},
		{
			name:     "USER.FACADE.IDENTIFIER_EMPTY returns 400",
			code:     domainerrors.ErrorCodeUserFacadeIdentifierEmpty,
			expected: 400,
		},
		{
			name:     "USER.FACADE.PACK_NOT_FOUND returns 404",
			code:     domainerrors.ErrorCodeUserFacadePackNotFound,
			expected: 404,
		},
		{
			name:     "USER.FACADE.WORKFLOW_NOT_FOUND returns 404",
			code:     domainerrors.ErrorCodeUserFacadeWorkflowNotFound,
			expected: 404,
		},
		{
			name:     "USER.FACADE.SESSION_CLOSED returns 409",
			code:     domainerrors.ErrorCodeUserFacadeSessionClosed,
			expected: 409,
		},
		{
			name:     "USER.FACADE.DUPLICATE_RESPONSE returns 409",
			code:     domainerrors.ErrorCodeUserFacadeDuplicateResponse,
			expected: 409,
		},
		{
			name:     "WORKFLOW.PARSE.YAML_SYNTAX returns 422",
			code:     domainerrors.ErrorCodeWorkflowParseYAMLSyntax,
			expected: 422,
		},
		{
			name:     "WORKFLOW.VALIDATION.CYCLE_DETECTED returns 422",
			code:     domainerrors.ErrorCodeWorkflowValidationCycleDetected,
			expected: 422,
		},
		{
			name:     "EXECUTION.COMMAND.TIMEOUT returns 503",
			code:     domainerrors.ErrorCodeExecutionCommandTimeout,
			expected: 503,
		},
		{
			name:     "SYSTEM.IO.READ_FAILED returns 500",
			code:     domainerrors.ErrorCodeSystemIOReadFailed,
			expected: 500,
		},
		{
			name:     "SYSTEM.IO.WRITE_FAILED returns 500",
			code:     domainerrors.ErrorCodeSystemIOWriteFailed,
			expected: 500,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := application.HTTPStatus(tt.code)
			assert.Equal(t, tt.expected, result, "should map %s to HTTP status %d", tt.code, tt.expected)
		})
	}
}

func TestHTTPStatus_CategoryMapping(t *testing.T) {
	t.Run("USER.INPUT codes return 400", func(t *testing.T) {
		...
		for _, code := range codes {
			t.Run(string(code), func(t *testing.T) {
				assert.Equal(t, 400, application.HTTPStatus(code))
			})
		}
	})

	t.Run("USER.FACADE.*_NOT_FOUND codes return 404", func(t *testing.T) {
		codes := []domainerrors.ErrorCode{
			domainerrors.ErrorCodeUserFacadePackNotFound,
			domainerrors.ErrorCodeUserFacadeWorkflowNotFound,
		}
		for _, code := range codes {
			t.Run(string(code), func(t *testing.T) {
				assert.Equal(t, 404, application.HTTPStatus(code))
			})
		}
	})

	t.Run("WORKFLOW category codes return 422", func(t *testing.T) {
		codes := []domainerrors.ErrorCode{
			domainerrors.ErrorCodeWorkflowParseYAMLSyntax,
			domainerrors.ErrorCodeWorkflowValidationCycleDetected,
		}
		for _, code := range codes {
			t.Run(string(code), func(t *testing.T) {
				assert.Equal(t, 422, application.HTTPStatus(code))
			})
		}
	})

	t.Run("SYSTEM category codes return 500", func(t *testing.T) {
		codes := []domainerrors.ErrorCode{
			domainerrors.ErrorCodeSystemIOReadFailed,
			domainerrors.ErrorCodeSystemIOWriteFailed,
		}
		for _, code := range codes {
			t.Run(string(code), func(t *testing.T) {
				assert.Equal(t, 500, application.HTTPStatus(code))
			})
		}
	})
}

func TestRPCCode_HappyPath(t *testing.T) {
	tests := []struct {
		name     string
		code     domainerrors.ErrorCode
		expected string
		...
		{
			name:     "USER.INPUT.MISSING_FILE returns INVALID_ARGUMENT",
			code:     domainerrors.ErrorCodeUserInputMissingFile,
			expected: "INVALID_ARGUMENT",
		},
		{
			name:     "USER.FACADE.IDENTIFIER_EMPTY returns INVALID_ARGUMENT",
			code:     domainerrors.ErrorCodeUserFacadeIdentifierEmpty,
			expected: "INVALID_ARGUMENT",
		},
		{
			name:     "USER.FACADE.IDENTIFIER_MALFORMED returns INVALID_ARGUMENT",
			code:     domainerrors.ErrorCodeUserFacadeIdentifierMalformed,
			expected: "INVALID_ARGUMENT",
		},
		{
			name:     "USER.FACADE.PACK_NOT_FOUND returns NOT_FOUND",
			code:     domainerrors.ErrorCodeUserFacadePackNotFound,
			expected: "NOT_FOUND",
		},
		{
			name:     "USER.FACADE.WORKFLOW_NOT_FOUND returns NOT_FOUND",
			code:     domainerrors.ErrorCodeUserFacadeWorkflowNotFound,
			expected: "NOT_FOUND",
		},
		{
			name:     "USER.FACADE.SESSION_CLOSED returns FAILED_PRECONDITION",
			code:     domainerrors.ErrorCodeUserFacadeSessionClosed,
			expected: "FAILED_PRECONDITION",
		},
		{
			name:     "USER.FACADE.INPUT_REJECTED returns INVALID_ARGUMENT",
			code:     domainerrors.ErrorCodeUserFacadeInputRejected,
			expected: "INVALID_ARGUMENT",
		},
		{
			name:     "USER.FACADE.DUPLICATE_RESPONSE returns FAILED_PRECONDITION",
			code:     domainerrors.ErrorCodeUserFacadeDuplicateResponse,
			expected: "FAILED_PRECONDITION",
		},
		{
			name:     "WORKFLOW.PARSE.YAML_SYNTAX returns INVALID_ARGUMENT",
			code:     domainerrors.ErrorCodeWorkflowParseYAMLSyntax,
			expected: "INVALID_ARGUMENT",
		},
		{
			name:     "WORKFLOW.VALIDATION.CYCLE_DETECTED returns INVALID_ARGUMENT",
			code:     domainerrors.ErrorCodeWorkflowValidationCycleDetected,
			expected: "INVALID_ARGUMENT",
		},
		{
			name:     "EXECUTION.COMMAND.TIMEOUT returns DEADLINE_EXCEEDED",
			code:     domainerrors.ErrorCodeExecutionCommandTimeout,
			expected: "DEADLINE_EXCEEDED",
		},
		{
			name:     "EXECUTION.COMMAND.FAILED returns INTERNAL",
			code:     domainerrors.ErrorCodeExecutionCommandFailed,
			expected: "INTERNAL",
		},
		{
			name:     "SYSTEM.IO.READ_FAILED returns INTERNAL",
			code:     domainerrors.ErrorCodeSystemIOReadFailed,
			expected: "INTERNAL",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := application.RPCCode(tt.code)
			assert.Equal(t, tt.expected, result, "should map %s to RPC code %s", tt.code, tt.expected)
		})
	}
}

func TestRPCCode_MapsToValidGRPCCodes(t *testing.T) {
	validGRPCCodes := map[string]bool{
		"OK":                  true,
		"CANCELLED":           true,
		"UNKNOWN":             true,
	...
	for _, code := range codes {
		t.Run(string(code), func(t *testing.T) {
			result := application.RPCCode(code)
			assert.NotEmpty(t, result, "RPC code should not be empty")
			assert.True(t, validGRPCCodes[result], "%s is not a valid gRPC code", result)
		})
	}
}
...
func TestErrorCodeMapping_Comprehensive(t *testing.T) {
	...
	t.Run("HTTPStatus never returns invalid values", func(t *testing.T) {
		codes := []domainerrors.ErrorCode{
			domainerrors.ErrorCodeUserInputMissingFile,
			domainerrors.ErrorCodeUserFacadePackNotFound,
			domainerrors.ErrorCodeWorkflowParseYAMLSyntax,
		...
		for _, code := range codes {
			result := application.HTTPStatus(code)
			assert.GreaterOrEqual(t, result, 400, "HTTP status should be >= 400")
			assert.Less(t, result, 600, "HTTP status should be < 600")
		}
	})

	t.Run("RPCCode never returns empty", func(t *testing.T) {
		codes := []domainerrors.ErrorCode{
			domainerrors.ErrorCodeUserInputMissingFile,
			domainerrors.ErrorCodeUserFacadePackNotFound,
			domainerrors.ErrorCodeWorkflowParseYAMLSyntax,
		...
		for _, code := range codes {
			result := application.RPCCode(code)
			assert.NotEmpty(t, result, "RPC code should not be empty for %s", code)
		}
	})
}
