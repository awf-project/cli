...
func TestVerifyChecksum(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:      "valid checksum",
			data:      []byte("test data"),
			checksum:  fmt.Sprintf("%x", sha256.Sum256([]byte("test data"))),
			wantError: false,
		},
		{
		...
		},
		{
			...
			data:      []byte{},
			checksum:  fmt.Sprintf("%x", sha256.Sum256([]byte{})),
			wantError: false,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			installer := pluginmgr.NewPluginInstaller()
			err := installer.VerifyChecksum(tt.data, tt.checksum)
			if tt.wantError {
				assert.Error(t, err)
			} else {
...
func TestDownload(t *testing.T) {
	t.Run("successful download", func(t *testing.T) {
		...
		data, err := installer.Download(ctx, server.URL+"/plugin.tar.gz", checksum)
		assert.NoError(t, err)
	...
	t.Run("context cancellation", func(t *testing.T) {
		...
		_, err := installer.Download(ctx, "https://example.com/plugin.tar.gz", "checksum")
		assert.Error(t, err)
	})
}
...
func TestExtractTarGz(t *testing.T) {
	t.Run("successful extraction", func(t *testing.T) {
		...
		tempDir := t.TempDir()
		installer := pluginmgr.NewPluginInstaller()
		err = installer.ExtractTarGz(buf.Bytes(), tempDir)
		assert.NoError(t, err)
	...
	t.Run("invalid tar.gz", func(t *testing.T) {
		...
		err := installer.ExtractTarGz([]byte("not a valid tarball"), tempDir)
		assert.Error(t, err)
	})
}
