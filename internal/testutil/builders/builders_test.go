	...
	tests := []struct {
		...
		},
		{
			...
			buildFunc: func() *application.ExecutionService {
				...
				builder = builder.WithLogger(mocks.NewMockLogger())
				builder = builder.WithExecutor(mocks.NewMockCommandExecutor())
...
func TestExecutionServiceBuilder_FluentAPI(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			buildFunc: func() *application.ExecutionService {
				builder := NewExecutionServiceBuilder()
				builder2 := builder.WithLogger(mocks.NewMockLogger())
				builder3 := builder2.WithExecutor(mocks.NewMockCommandExecutor())
		...
		},
		{
			...
			buildFunc: func() *application.ExecutionService {
				return NewExecutionServiceBuilder().
					WithStateStore(mocks.NewMockStateStore()).
					WithLogger(mocks.NewMockLogger()).
					WithWorkflowRepository(mocks.NewMockWorkflowRepository()).
...
func TestExecutionServiceBuilder_AgentRegistryDefault_EdgeCases(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			buildFunc: func() *application.ExecutionService {
				builder := NewExecutionServiceBuilder()
				builder = builder.WithAgentRegistry(mocks.NewMockAgentRegistry())
